...
class ImportJob(models.Model):
    file = ImportExportFileField(
        ...
        upload_to="django-import-export-huey-import-jobs",
        blank=False,
        null=False,
        max_length=255,
    )
    ...
    change_summary = ImportExportFileField(
        ...
        upload_to="django-import-export-huey-import-change-summaries",
        blank=True,
        null=True,
    )
    ...
    def get_format_choices():
        ...
        from import_export_huey.utils import get_formats
        return [
            (f.CONTENT_TYPE, f().get_title())
            for f in DEFAULT_FORMATS
            if f().can_import() and f in get_formats()
        ]
