...
def lazy_initialize_storage_class():
    ...
    if not hasattr(settings, 'IMPORT_EXPORT_HUEY_STORAGE') and storages_defined:
        # Use new style storages if defined
        storage_alias = getattr(settings, "IMPORT_EXPORT_HUEY_STORAGE_ALIAS", "default")
        storage_class = storages[storage_alias]
    else:
        # Use old style storages if defined
        from django.core.files.storage import get_storage_class
        storage_class = get_storage_class(getattr(settings, "IMPORT_EXPORT_HUEY_STORAGE", "django.core.files.storage.FileSystemStorage"))
