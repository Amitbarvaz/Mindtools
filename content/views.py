...
@ratelimit(group='post_by_user', key='user_or_ip', rate='90/m', method=UNSAFE, block=True)
@ratelimit(group='post_by_user', key='user_or_ip', rate='1000/d', method=UNSAFE, block=True)
...
def _escape_html(text):
    ...
    return re.sub(r'<[^>]+>', lambda match: re.sub(r'[<>]', lambda x: map[x.group()], match.group()), text)

@login_required
@ratelimit(group='post_by_user', key='user_or_ip', rate='90/m', method=UNSAFE, block=True)
@ratelimit(group='post_by_user', key='user_or_ip', rate='100/h', method=UNSAFE, block=True)
@ratelimit(group='post_by_user', key='user_or_ip', rate='500/d', method=UNSAFE, block=True)
...
def receive_messages_internal(prev, next, current_user, other_user):
    ...
    objects = {
        'messages': [{'id': m.id, 'msg': _processTextMessage(m.message), 'time': m.timestamp,
