...
if (typeof(ChatController) !== 'undefined') {
    therapistapp.controller('ChatController', ['$scope', '$http', '$httpParamSerializerJQLike', '$timeout', '$interval', '$sce', ChatController()]);
...
therapistapp.controller('therapist', ['$rootScope', '$scope', '$http', '$httpParamSerializerJQLike', '$timeout', '$interval', function (rootScope, scope, http, httpParamSerializerJQLike, timeout, interval) {
    ...
    scope.postVariables = function () {
        var url = api + '?user_id=' + scope.user_id;
        let data = {};
        for (var variableName in scope.variablesForm.data) {
            const currentValue = scope.variablesForm.data[variableName];
            const previousValue = scope.variablesForm.fields.filter((field) => field['name'] === variableName)[0]['value'];
            if (currentValue !== previousValue && !(currentValue === '' && previousValue === null)) {
                data = {...data, [variableName]: currentValue };
            }
        }
...
if (typeof(ChatController) !== 'undefined') {
    mytherapistapp.controller('ChatController', ['$scope', '$http', '$httpParamSerializerJQLike', '$timeout', '$interval', '$sce', ChatController()]);
